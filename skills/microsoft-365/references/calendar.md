...
## Get event details
```bash
python3 $SKILL_DIR/scripts/ms_graph.py calendar get --event-id <id>
```

## Update event
```bash
python3 $SKILL_DIR/scripts/ms_graph.py calendar update \
  --event-id <id> \
...
## List calendars
```bash
python3 $SKILL_DIR/scripts/ms_graph.py calendar calendars
```
...
python3 $SKILL_DIR/scripts/ms_graph.py calendar share-list [--calendar-id <id>]

# Add sharing permission
python3 $SKILL_DIR/scripts/ms_graph.py calendar share-add \
...
python3 $SKILL_DIR/scripts/ms_graph.py calendar share-update \
  --permission-id <id> --role <role> [--calendar-id <id>]

# Remove sharing permission
python3 $SKILL_DIR/scripts/ms_graph.py calendar share-remove \
